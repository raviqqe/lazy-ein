		...
		// Function applications
		{
			ast.NewBind(
				...
				ast.NewLambda(
					...
					ast.NewApplication(ast.NewVariable("x"), []ast.Atom{ast.NewFloat64(42)}),
					types.NewFloat64(),
				),
			),
		},
		...
		// Mutually recursive global variables
		{
			ast.NewBind(
				...
				ast.NewLambda(
					...
					true,
					nil,
					ast.NewApplication(ast.NewVariable("bar"), nil),
					types.NewBoxed(types.NewFloat64()),
				),
			),
			ast.NewBind(
		...
		// Recursive functions
		{
			ast.NewBind(
				...
				ast.NewLambda(
					...
					false,
					[]ast.Argument{ast.NewArgument("x", types.NewFloat64())},
					ast.NewApplication(ast.NewVariable("foo"), []ast.Atom{ast.NewVariable("x")}),
					types.NewFloat64(),
				),
			),
		},
			...
			ast.NewBind(
				...
				ast.NewLambda(
					...
					ast.NewLet(
						...
						ast.NewApplication(ast.NewVariable("bar"), []ast.Atom{ast.NewVariable("x")}),
					),
					types.NewBoxed(types.NewFloat64()),
				),
			),
